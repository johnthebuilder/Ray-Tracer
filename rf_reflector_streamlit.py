...
def main():
    ...
    with st.sidebar.expander("ℹ️ About This Simulation"):
        ...
        **Animation Features:**
        - Watch catacaustic collapse from envelope to point/ring
        - Export as GIF for presentations
        - Zoomed view shows fine detail of collapse
        """)
    ...
    with col1:
        ...
        if st.session_state.get('create_gif', False):
            st.subheader("🎬 Creating Animation GIF")
...
def main():
    ...
    with col2:
        ...
        if analyze_ray:
            st.subheader("🔬 Ray Path Analysis")
            angle_to_use = incident_angle if not animate_angles else st.session_state.get('angle_values', [90])[st.session_state.get('current_frame', 0)]
            ray_data = compute_ray_path(angle_to_use, focal_length, hole_radius, ray_radius, ray_theta)
